<template>
  <div>
    <section id="posts" class="flex md:flex-row flex-col items-center justify-between">
      <p class="font-serif text-4xl text-brand-dark md:pb-0 pb-6">Most Recent Blog Posts</p>
      <div class="flex sm:flex-row flex-col items-center">
        <div class="sm:mr-4 sm:pb-0 pb-4">
          <categories @category-clicked="categoryClicked" :categories="uniqueCategories"></categories>
        </div>
        <sort @sort="sort"></sort>
      </div>
    </section>
    ...
    <Transition>
      <section v-if="activeCategories.length > 0" class="flex md:flex-row flex-col items-center pt-6">
        <p class="text-sm uppercase tracking-md md:mr-4 md:pb-0 pb-4">categories selected:</p>
        <div class="flex flex-wrap gap-4">
          <div v-for="category in activeCategories" @click="removeCategory(category)" class="flex items-center px-4 py-2 bg-brand-100 rounded-100 hover:bg-brand-dark transition-fast cursor-pointer">
            <p class="capitalize text-sm text-white">{{ category.replaceAll('-', ' ') }}</p>
            <svg class="ml-3 w-4 h-4" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ffffff" fill="none" stroke-linecap="round" stroke-linejoin="round">
    ...
    <section class="grid lg:grid-cols-3 sm:grid-cols-2 gap-8" :class="{'py-20':activeCategories.length === 0, 'pb-20 pt-4':activeCategories.length > 0}">
        <post v-for="post in paginatedPosts.slice(0, 3)" :post="post" :key="post.filename"></post>
    </section>
    ...
    <section class="py-16 border border-brand-grey-250 rounded-12">
      <div class="flex md:flex-row flex-col items-center justify-between md:w-4/5 w-mobile mx-auto">
        <p class="font-serif text-4xl text-brand-dark md:pb-0 pb-6 md:text-left text-center">Subscribe to Our Blog</p>
...
export default {
  ...
  mounted() {
    this.blogPosts = this.posts.filter( post => !post.categories.includes('newsletter'))
  },
  computed: {
    ...
    showablePosts: function() {
      ...
      let showablePosts = this.blogPosts.filter( post => {
        return post.categories.some( category => this.activeCategories.includes(category));
      });

      showablePosts = showablePosts.length ? showablePosts : this.blogPosts;

      switch (this.sortBy) {
        case "A - Z":
          showablePosts.sort(function(a, b) {
        ...
        case "Z - A":
          ...
          break;
        case 'LATEST':
          showablePosts.sort((a, b) => new Date(b.date) - new Date(a.date))
      }
